...
export async function sha256Hex(value) { const hash = await crypto.subtle.digest("SHA-256", encoder.encode(value)); return [...new Uint8Array(hash)].map(b=>b.toString(16).padStart(2,"0")).join(""); }
...
export function getCookie(request, name) { const cookie = request.headers.get("cookie") || ""; const match = cookie.match(new RegExp(`(?:^|; )${name}=([^;]*)`)); return match ? decodeURIComponent(match[1]) : ""; }
...
export async function ensureSchema(env) {
  ...
  const statements = [
    `CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, first_name TEXT NOT NULL, last_name TEXT NOT NULL, username TEXT NOT NULL UNIQUE COLLATE NOCASE, password_hash TEXT NOT NULL, salt TEXT NOT NULL, role TEXT NOT NULL DEFAULT 'user', created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)`,
    `CREATE TABLE IF NOT EXISTS sessions (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL, token_hash TEXT NOT NULL UNIQUE, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, expires_at TEXT NOT NULL, FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE)`,
    ...
    `CREATE TABLE IF NOT EXISTS categories (id INTEGER PRIMARY KEY AUTOINCREMENT, parent_id INTEGER, title TEXT NOT NULL, slug TEXT NOT NULL UNIQUE, description TEXT DEFAULT '', sort_order INTEGER DEFAULT 0, status TEXT DEFAULT 'published', created_at TEXT DEFAULT CURRENT_TIMESTAMP, updated_at TEXT DEFAULT CURRENT_TIMESTAMP)`,
    `CREATE TABLE IF NOT EXISTS notes (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT NOT NULL, slug TEXT NOT NULL UNIQUE, summary TEXT DEFAULT '', content_html TEXT NOT NULL DEFAULT '', category_id INTEGER, status TEXT DEFAULT 'draft', views INTEGER DEFAULT 0, created_at TEXT DEFAULT CURRENT_TIMESTAMP, updated_at TEXT DEFAULT CURRENT_TIMESTAMP, published_at TEXT, FOREIGN KEY(category_id) REFERENCES categories(id) ON DELETE SET NULL)`,
    `CREATE TABLE IF NOT EXISTS note_versions (id INTEGER PRIMARY KEY AUTOINCREMENT, note_id INTEGER NOT NULL, title TEXT NOT NULL, summary TEXT DEFAULT '', content_html TEXT NOT NULL, created_at TEXT DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY(note_id) REFERENCES notes(id) ON DELETE CASCADE)`,
...
export async function seedIfEmpty(env) {
  ...
  for (const [i,title] of ['Unit 1: Basic Economic Concepts','Unit 2: Supply and Demand','Unit 3: Production, Cost, and Perfect Competition','Unit 4: Imperfect Competition','Unit 5: Factor Markets','Unit 6: Market Failure'].entries()) {
  ...
  await env.DB.prepare("INSERT INTO notes (title,slug,summary,content_html,category_id,status,published_at) VALUES (?,?,?,?,?,'published',CURRENT_TIMESTAMP)").bind(
    ...
    `<h2>Supply and Demand</h2><p>Demand shifts when non-price determinants change. Supply shifts when input costs, technology, taxes, subsidies, or seller expectations change.</p><ul><li>Binding price ceiling: shortage.</li><li>Binding price floor: surplus.</li><li>Per-unit tax: quantity decreases and creates deadweight loss.</li></ul><h2>Elasticity</h2><p>Elastic demand means quantity responds strongly to price changes. Inelastic demand means quantity responds weakly.</p>`,
    unit2.id
  ).run();
  const note = await env.DB.prepare("SELECT id FROM notes WHERE slug='supply-demand-elasticity-cheat-sheet'").first();
  await env.DB.prepare("INSERT INTO flashcards (note_id,front,back,sort_order) VALUES (?,?,?,0),(?,?,?,1)").bind(note.id,'Binding price ceiling?','Shortage if set below equilibrium.',note.id,'Per-unit tax effect?','Raises buyer price, lowers seller received price, decreases quantity.').run();
  await env.DB.prepare("INSERT INTO practice_questions (note_id,question,options_json,correct_answer,explanation) VALUES (?,?,?,?,?)").bind(note.id,'A binding price floor usually causes what?','["Shortage","Surplus","No change","Demand increase"]','Surplus','A binding floor is above equilibrium, so quantity supplied exceeds quantity demanded.').run();
  for (const tag of ['AP Micro','Elasticity','Supply and Demand']) {
    ...
    await env.DB.prepare("INSERT OR IGNORE INTO note_tags (note_id,tag_id) VALUES (?,?)").bind(note.id,t.id).run();
  }
  for (const [order,title,links] of [
    ...
    for (let i=0;i<links.length;i++) await env.DB.prepare("INSERT INTO footer_links (column_id,label,url,sort_order) VALUES (?,?,?,?)").bind(col.id,links[i][0],links[i][1],i).run();
  }
}

export async function createSession(env, userId) { await ensureSchema(env); const token=randomHex(32); const tokenHash=await sha256Hex(token); const expiresAt=new Date(Date.now()+SESSION_DAYS*86400*1000).toISOString(); await env.DB.prepare("INSERT INTO sessions (user_id, token_hash, expires_at) VALUES (?, ?, ?)").bind(userId, tokenHash, expiresAt).run(); return token; }
export async function getUserFromRequest(request, env) {
  await ensureSchema(env);
  const token = getCookie(request, SESSION_COOKIE); if (!token) return null;
  const tokenHash = await sha256Hex(token);
  const row = await env.DB.prepare(`SELECT users.id, users.first_name, users.last_name, users.username, COALESCE(users.role,'user') AS role FROM sessions JOIN users ON users.id=sessions.user_id WHERE sessions.token_hash=? AND sessions.expires_at > datetime('now') LIMIT 1`).bind(tokenHash).first();
  return row || null;
}
export async function requireUser(request, env) { const user = await getUserFromRequest(request, env); if (!user) return { error: json({ ok:false, error:"Not signed in" }, 401) }; return { user }; }
export async function requireAdmin(request, env) { const auth = await requireUser(request, env); if (auth.error) return auth; if (auth.user.role !== 'admin') return { error: json({ ok:false, error:"Admin access required" }, 403) }; return auth; }
