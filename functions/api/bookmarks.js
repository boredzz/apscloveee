...
export async function onRequestGet({ request, env }) {
  ...
  const rows = (await env.DB.prepare("SELECT n.id,n.title,n.slug,n.summary,b.created_at FROM bookmarks b JOIN notes n ON n.id=b.note_id WHERE b.user_id=? ORDER BY b.created_at DESC").bind(auth.user.id).all()).results || [];
