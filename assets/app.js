...
function showBookmarks(){const saved=JSON.parse(localStorage.ap_bookmarks||'[]');const cards=state.notes.filter(n=>saved.includes(n.slug)).map(noteCard).join('');view(`<h2>Saved Notes</h2><div class='grid'>${cards||"<p class='muted'>No bookmarks yet.</p>"}</div>`)}
